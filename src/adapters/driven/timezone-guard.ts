 ...
 *
 * ★ และบน Vercel "ไปตั้งที่ host" ไม่ใช่ทางเลือกด้วยซ้ำ — TZ อยู่ในรายชื่อที่ Vercel
 *   สงวนไว้ หน้า dashboard ปฏิเสธตรงๆ ว่า "The name of your Environment Variable
 *   is reserved." function จึงรันเป็น UTC เสมอและเปลี่ยนจากข้างนอกไม่ได้เลย
 *   ทางนี้ไม่ได้เป็นแค่ตาข่ายรองรับ แต่เป็นทางเดียวที่เหลืออยู่จริงบน host นี้
 *   ถ้าวันหนึ่งย้าย host แล้วตั้ง TZ ได้ ค่อยตั้งเพิ่ม ไม่ต้องรื้อไฟล์นี้ทิ้ง
 *
 * ★ แต่ต้องไม่กลายเป็น "ผิดเงียบๆ" ซึ่งเป็นสิ่งที่ไฟล์นี้มีไว้กันตั้งแต่แรก
 *   จึงยังดังอยู่สองระดับ: ซ่อมได้ก็ log ให้เห็นว่าเกิดอะไรขึ้นและซ่อมด้วยอะไร
 ...
 *
 * ซ่อมได้ → log หนึ่งบรรทัด (บน Vercel เป็นเรื่องปกติ ไม่ต้องตามแก้) ส่วน host ที่ตั้ง TZ ได้
 * ควรไปตั้งไว้ด้วย เพราะค่าที่ตั้งตรงนี้มีผลเฉพาะ process ที่เรียกฟังก์ชันนี้เท่านั้น
 *
...
export function ensureAppTimezone(
  ...
  if (outcome.kind === "corrected") {
    env.warn(
      `[timezone] process เริ่มมาด้วย "${outcome.from ?? "ไม่ทราบ"}" — ตั้งเป็น ${APP_TIMEZONE} ให้แล้ว\n` +
        `  บน Vercel ข้อความนี้เป็นเรื่องปกติ ไม่ต้องตามแก้ — TZ เป็นชื่อที่ Vercel สงวนไว้\n` +
        `  ตั้งใน environment variables ไม่ได้ ทางนี้จึงเป็นทางเดียวที่มี\n` +
        `  ส่วน host ที่ตั้ง TZ=${APP_TIMEZONE} ได้ ควรตั้งไว้ด้วย เพราะค่าที่ตั้งตรงนี้\n` +
        `  มีผลเฉพาะ process ที่เรียกฟังก์ชันนี้ สคริปต์ที่ไม่ผ่าน register() จะไม่ได้ค่านี้`,
    );
    return outcome;
  }
  ...
  const detail =
    ...
    `  host ที่ตั้ง TZ=${APP_TIMEZONE} ได้ ให้ไปตั้งที่ environment variables ของ host\n` +
    `  ส่วนบน Vercel ตั้งไม่ได้ เพราะ TZ เป็นชื่อสงวน เคสนี้จึงแปลว่า runtime เองมีปัญหา\n` +
    `  ต้องแก้ที่ runtime หรือย้าย host ไม่ใช่ตามหา environment variable ที่ลืมตั้ง\n` +
